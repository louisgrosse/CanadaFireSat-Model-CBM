"""
This python file generates a specified number of images from the CanadaFireSat dataset labeled
with the corresponding name for MS-CLIP, MS-ClearCLIP or MS-SCLIP depending on the config. It also 
logs the entropy and number of times each name from the selected dictionary is selected as the best
for analysis.
"""
...
"""
This function generates a graph with the original image, the fire labeled image and the patch named image.
"""
    ...
    for caption in cfg["captions_glob"]:
        ...
        for bi, batch in enumerate(tqdm(loader)):
            ...
            if len(pos_idx_list) > 0 and plotted_batches < cfg["n_show_batches"]:
                ...
                y_bin  = _binarize_labels(y_full, input_["inputs"])  # -> [B,1,Hc,Wc]
        ...
        if n_tokens > 0:
            mean_unit = (sum_unit / n_tokens).float()   # mean of unit patch vectors
            mean_raw  = (sum_raw  / n_tokens).float()   # mean of raw patch embeddings

            avg_cos = sum_cos / n_tokens  # mean_i cos(p_i, phrase)
